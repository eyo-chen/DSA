...
func NewMinHeapNode(size int) *MinHeapNode {
	return &MinHeapNode{
		vals: make([]*ListNode, 0, size),
	}
}

func (m *MinHeapNode) Len() int {
	return len(m.vals)
}
...
func (m *MinHeapNode) Insert(val *ListNode) {
...
func (m *MinHeapNode) Pull() *ListNode {
...
func (m *MinHeapNode) siftUp(i int) {
	for i > 0 {
...
func (m *MinHeapNode) siftDown(i int) {
	for {
		minIndex := i
		...
		if left < len(m.vals) && m.vals[left].Val < m.vals[minIndex].Val {
			minIndex = left
		}
		if right < len(m.vals) && m.vals[right].Val < m.vals[minIndex].Val {
			minIndex = right
		}
...
func (m *MinHeapNode) swap(i, j int) {
	m.vals[i], m.vals[j] = m.vals[j], m.vals[i]
}

func (m *MinHeapNode) getParentIndex(i int) int {
	return (i - 1) / 2
}
