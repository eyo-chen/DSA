...
func NewMaxHeapInt(size int) *MaxHeapInt {
	return &MaxHeapInt{
		vals: make([]int, 0, size),
	}
}

func (m *MaxHeapInt) Len() int {
	return len(m.vals)
}
...
func (m *MaxHeapInt) Insert(val int) {
...
func (m *MaxHeapInt) siftUp(i int) {
	for i > 0 {
...
func (m *MaxHeapInt) siftDown(i int) {
	for {
		maxIndex := i
		...
		if left < len(m.vals) && m.vals[left] > m.vals[maxIndex] {
			maxIndex = left
		}
		if right < len(m.vals) && m.vals[right] > m.vals[maxIndex] {
			maxIndex = right
		}
...
func (m *MaxHeapInt) getParentIndex(i int) int {
	return (i - 1) / 2
}

func (m *MaxHeapInt) swap(i, j int) {
